...
public class BraunschweigCarPassengerPredictor extends CachedVariablePredictor<BraunschweigCarPassengerVariables> {
	@Override
	public BraunschweigCarPassengerVariables predict(Person person, DiscreteModeChoiceTrip trip,
		...
		for (Leg leg : TripStructureUtils.getLegs(elements)) {
			if (leg.getMode().equals(BraunschweigModeChoiceModule.CAR_PASSENGER)) {
		...
		return new BraunschweigCarPassengerVariables(passengerTravelTime_min, euclideanDistance_km, accessEgressTime_min);
	}
}